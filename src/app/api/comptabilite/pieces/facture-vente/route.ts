...
export const POST = withApi(async (req: NextRequest) => {
  ...
  const { ecriture, generee } = await enregistrerFactureVente(body, user.id);
  ...
  await writeAudit({
    ...
    diff: { apres: { origine: "FACTURE_VENTE", reference: generee.reference, totalTtc: generee.totalTtc } },
    ip: clientIp(req),
  });
